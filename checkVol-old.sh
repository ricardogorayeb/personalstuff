...
for ARQFULL in $(ls $DIRVMET/*.mvol | tail -n850);do
	...
	if [ $NUMVOL -eq 17 ]; then
		...
		if [ -s $DIRBASEVMET/$ANO/$MES/$DIA/${NOMEVOL}.mvol.bz2 ]; then
			...
			if [ ! -d $DIRBASEVMET/$ANO ]; then mkdir -p $DIRBASEVMET/$ANO; fi
			if [ ! -d $DIRBASEVMET/$ANO/$MES ]; then mkdir -p $DIRBASEVMET/$ANO/$MES; fi
			if [ ! -d $DIRBASEVMET/$ANO/$MES/$DIA ]; then mkdir -p $DIRBASEVMET/$ANO/$MES/$DIA; fi
...
for ARQ in $(ls $DIRXPPI | tail -n350);do
	...
	echo "$NUMVOL"
		...
		if [ -s $DIRBASEXPPI/$ANO/$MES/$DIA/${NOMEVOL}_400.mvol.bz2 ]; then
			...
			if [ ! -d $DIRBASEXPPI/$ANO ]; then mkdir -p $DIRBASEXPPI/$ANO; fi
			if [ ! -d $DIRBASEXPPI/$ANO/$MES ]; then mkdir -p $DIRBASEXPPI/$ANO/$MES; fi
			if [ ! -d $DIRBASEXPPI/$ANO/$MES/$DIA ]; then mkdir -p $DIRBASEXPPI/$ANO/$MES/$DIA; fi
			echo "COPIANDO ARQUIVO "  $ARQ
